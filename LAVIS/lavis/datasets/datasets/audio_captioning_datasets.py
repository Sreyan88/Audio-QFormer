...
class AudioLanguagePretrainDataset_CK(BaseDataset, __DisplMixin):
    def __init__(self, file_path, text_processors):
        # json_files = kwargs['ann_paths'][:-1]
        # blacklist = None
        # self._load_json_file(json_files, kwargs["audio_root"], blacklist)
        self.annotation = pd.read_csv(file_path, sep=',')
        self.audios = self.annotation['path'].tolist()
        self.captions = self.annotation['caption'].tolist()
        self.captions = [eval(cap_list) for cap_list in self.captions]
        self.datasets = self.annotation['dataset'].tolist()

        self.cached = False
        self.cache_dir = ''
        self.text_processor = text_processors

        self.txt2img = {}
        self.img2txt = {}

        # grouped_df = self.annotation.groupby('path').agg(lambda x: list(x)).reset_index()

        # path_dict = grouped_df.set_index('path').T.to_dict()

        # print(path_dict)

        txt_id = 0
        for img_id, audio in enumerate(self.audios):
            self.img2txt[img_id] = []
            # print(path_dict[audio]["caption"])
            for _, _ in enumerate(self.captions[img_id]):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            waveform, sr = torch.zeros(160000), 16000
        ...
        caption = [self.text_processor["eval"](caption) for caption in self.captions[index]]
        audio_id = self.captions[index][0] + "_" + str(index)
...
class AudioLanguagePretrainDataset(BaseDataset, __DisplMixin):
    def __init__(self, **kwargs):
        ...
        # blacklist = None
        # self._load_json_file(json_files, kwargs["audio_root"], blacklist)
        self.annotation = pd.read_csv('/fs/nexus-projects/brain_project/naacl_audio/zaratan_match_train.csv', sep=',')
        self.audios = self.annotation['path'].tolist()
        self.captions = self.annotation['caption'].tolist()
        self.datasets = self.annotation['dataset'].tolist()
    ...
    def __getitem__(self, index):
        ...
        caption = self.text_processor(self.captions[index])
        audio_id = self.captions[index] + "_" + str(index)
