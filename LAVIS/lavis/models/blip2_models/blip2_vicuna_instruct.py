...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            ...
            image_output = image_output.reshape(image_output.shape[0], 8, 64, image_output.shape[-1])
            image_output = torch.mean(image_output, dim=1)  # mean pool over the frequency dimension # [B, 64, 768]
            # hard norm to 50
            image_output = image_output / 50
            image_embeds = self.ln_vision(image_output)  #hidden_features
        ...
        empty_targets = (
            torch.ones(atts_llm.size(), dtype=torch.long).to(image_embeds.device).fill_(-100)
        )
    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            for j in range(image.size(2)):
                ...
                frame_atts = torch.ones(frame_embeds.size()[:-1], dtype=torch.long).to(image_embeds.device)
                ...
                frame_atts_llm = torch.ones(frame_inputs_llm.size()[:-1], dtype=torch.long).to(image_embeds.device)
