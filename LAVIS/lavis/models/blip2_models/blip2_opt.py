...
class Blip2OPT(Blip2Base):
    ...
    def forward(self, samples):
        ...
        atts_opt = torch.ones(inputs_opt.size()[:-1], dtype=torch.long).to(image_embeds.device)
        ...
        empty_targets = (
            torch.ones(atts_opt.size(), dtype=torch.long).to(image_embeds.device).fill_(-100)
        )
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            # hard norm to 50
            image_output = image_output / 50
            image_embeds = self.ln_vision(image_output)
            image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image_embeds.device)
