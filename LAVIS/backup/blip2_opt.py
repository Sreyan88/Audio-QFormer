...
class Blip2OPT(Blip2Base):
    ...
    def forward(self, samples):
        ...
        image = {"longer": samples["audio_longer"],
        "waveform": samples["audio_waveform"],
        "mel_fusion": samples["audio_mel_fusion"]
        }

        with self.maybe_autocast():
            image_output = self.visual_encoder(image)
            image_embeds = self.ln_vision(image_output['features'])  #hidden_features
    ...
    def generate(
        ...
        with self.maybe_autocast():
            image_output = self.visual_encoder(image)
            image_embeds = self.ln_vision(image_output['features'])
            image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image_embeds.device)
